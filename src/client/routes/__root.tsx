...
import {
	GlobalPageHeaderContext,
	type GlobalPageHeaderSlots,
} from "../pageHeaderSlots.ts";
...
function RootLayout() {
	...
	const [pageHeaderSlots, setPageHeaderSlotsState] =
		useState<GlobalPageHeaderSlots>({});
				...
				<div className="flex min-h-dvh flex-col lg:flex-row">
					<aside className="w-full border-b border-kumo-line bg-kumo-elevated lg:sticky lg:top-0 lg:h-dvh lg:w-68 lg:shrink-0 lg:border-r lg:border-b-0">
						<div className="border-b border-kumo-line px-4 py-4">
							...
							<h1 className="mt-1 text-base font-semibold text-kumo-default">
								Navigation
							</h1>
						</div>
...
function GlobalPageHeader({
	...
	const titleSlot = slots.title ?? (
		<h1 className="text-2xl font-semibold text-kumo-default md:text-3xl">
			Template Page
		</h1>
	);
	...
	return (
		<header className="border-b border-kumo-line bg-kumo-base">
			{slots.top ? (
				<div className="border-b border-kumo-line px-4 py-2 md:px-6 lg:px-8">
					{slots.top}
				</div>
			) : null}
			...
			{slots.tabs ? (
				<div className="border-t border-kumo-line px-4 py-3 md:px-6 lg:px-8">
					{slots.tabs}
				</div>
			) : null}
